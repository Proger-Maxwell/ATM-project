...
if(isset($_SESSION['isLogin'])) {
    ...
    if($_SERVER['REQUEST_METHOD']=='POST') {
        ...
        $user_data=$user->addMoney($_SESSION['bill_number'], $money);

        if( !isset($user_data['error'])) {


            header("Location: /user-page.php?notification=__add_money_success");
            exit();

        }

        else {
            header("Location: /add-money.php?notification=".$user_data['error']);
            exit();
        }
    }
        ...
        $view->text='ATM Банкинг / Пополнение счета';
        ...
        $view->main_data=$view->block('views/user-page/add-money-form.php');
